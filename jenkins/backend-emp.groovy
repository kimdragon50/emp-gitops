...
def gitOpsUrl = "https://git-codecommit.ap-northeast-2.amazonaws.com/v1/repos/${GIT_OPS}"
            ...
            steps {            
                script {
                    try {
                        ...
                        def cmd = "aws ecr list-images --repository-name ${BASE_ECR} --output text --query \"imageIds[?imageTag=='latest'].imageDigest\""
                        def digest =  executeCmdReturn(cmd)
                        
                        // digest 
                        sh("sed -i 's!<digest>.*!<digest>${digest}</digest>!g' /var/lib/jenkins/workspace/${JOB_NAME}/pom.xml")
                        
                        // sso properties file update
                        sh("cp /var/lib/jenkins/workspace/${JOB_NAME}/src/main/resources/sso_${CONFIG_ENV}.sso  /var/lib/jenkins/workspace/${JOB_NAME}/src/main/resources/sso.properties")
